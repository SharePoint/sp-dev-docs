...
| --- | --- |
| `ScanId` | Assessment identifier. |
| `WebPartType` | Assembly-qualified web part type. |
...
| `ScanId` | Assessment identifier. |
| `SiteUrl` | Absolute site-collection URL. |
| `PageUrl` | Server-relative page URL. On a site-level coverage row, this is the server-relative site URL. |
...
| `AuditUniqueUsers` | Distinct-user signal across the three operations, capped internally for highly used pages. |
...
| `ClassicPages` | Total discovered classic pages. |
...
The file also contains Classic Lists, Workflow compatibility, InfoPath, Extensibility, SharePoint Add-In, and Azure ACS rollup columns.
