...
> [!IMPORTANT]
> The legacy scanner is retained as-is for existing users. Its authentication flows, dependencies, and command examples might no longer represent current recommended practices. Test the scanner in a nonproduction environment before relying on it.
...
Since a typical scan needs to be able to scan all site collections, it's recommended to use an app-only principal with tenant-scoped permissions for the scan. This approach will ensure the scanner always has access. If you use an account (for example, your SharePoint tenant admin account), then the scanner can only access the sites where this user also has access. You can either use an Azure AD application or a SharePoint app principal for app-only access, and the links below describe the manual approach to setting up things. As the recommended approach is Azure AD App-Only, you can easy the setup up your Azure AD application by using the [PnP PowerShell](https://aka.ms/pnppowershell) `Register-PnPAzureADApp` cmdlet. See the **Using Register-PnPAzureADApp to set up Azure AD App-Only** chapter for more details.
...
> [!IMPORTANT]
> If you're choosing for Azure AD app-only, then above mentioned instructions grant the Azure AD app the **Sites.FullControl.All** permissions, which are needed if you want to run **all** scan components.
...
Once the preparation work is done, let's continue with a scan.

### Using Register-PnPAzureADApp to set up Azure AD App-Only
...
1. Use the `Register-PnPAzureADApp` cmdet to set up an Azure AD app:
...
1. You'll be asked to authenticate. Ensure you authenticate with a user who is a tenant administrator
1. The cmdlet will set up an Azure AD app and wait for 60 seconds to give Azure AD time to handle the App creation
1. You'll be asked to consent to the permissions granted to the created Azure AD application:
   1. First, you'll be asked to sign in again and use a tenant administrator account
   1. Nex,t the consent dialog is shown with the requested permissions (see screenshot below). Click **Accept**
1. Store the returned AzureAppId, created PFX file, and password somewhere safe, you'll need to use it as described in heading **Authenticate via Azure AD app-only**
...
- **Azure AD Domain name**: the default domain of your Azure AD environment. You can find this from the overview page of the Azure AD admin center. Typically, this domain is formatted as `*.onmicrosoft.com`, for example, c`ontoso.onmicrosoft.com`.
...
> [!NOTE]
> You can also opt to install the certificate on your computer and reference it via the `-w` command line parameter. To reference the certificate, you would use this value for the `-w` parameter `My|CurrentUser|1FG498B468AV3895E7659C8A6F098FB701C8CDB1`. You can use My/Root and CurrentUser/LocalMachine to identify the store. The last argument is the certificate thumbprint. This option is available as of the 2.7 release. If you're using LocalMachine then be aware that you'll need to ensure the scanner has permissions to read in the LocalMachine store. You can do this by either running the scanner process under administrative privileges or granting the account you're using to run the modernization scan permissions to read the private key of the certificate stored in the LocalMachine store.

> [!IMPORTANT]
> - The default instructions mentioned to grant the Azure AD app the **Sites.FullControl.All** permissions are needed if you want to run all scan components. If you're not interested in workflow scanning, you can also use **Sites.Read.All** as a permission as of version 2.6 of the SharePoint Modernization Scanner.
...
Using a regular user/password combo works fine as well, assuming the provided user does have the needed permissions. As of version 2.9 of the scanner, the user/password-based authentication does not rely anymore of legacy auth being enabled on the scanned tenant. The only reason this type of authentication would not work is when the user's account requires multi-factor authentication. If that's the case then please use the Multi Factor Auth option introduced as of version 2.9.
...
Scanning the complete tenant is often the recommended approach as that will give you modernization reports for all. If that's your choice, then simply fill in your tenant's name. This approach doesn't work if your tenant is **using URLs that do not end on sharepoint.com**, if that's the case you need to use one of the two options below.
...
- **A wildcard URL**: By adding a URL that ends with a star, you'll include all sites that match that filter. Only ending on an asterisk is a supported wildcard
...
- **Separator**: the default separator for the generated CSV files is a comma, but you can change this to semicolon if you would prefer that
