...
The [PnP provisioning engine](/sharepoint/dev/solution-guidance/pnp-provisioning-engine-and-the-core-library) can be used to deploy branding assets to SharePoint libraries. When using the Office 365 CDN capability, the files are automatically provisioned in the CDN. When you use alternative CDN solutions, a custom provisioning approach is needed to publish files to CDNs.
