...
![docs/embedded/images/CTCreationOverview.png](../images/CTCreationOverview.png)
...
SharePoint Embedded requires an Azure App Registration because applications—not end users—interact with SharePoint through Microsoft Graph. The app registration provides a secure identity for the application, defines the permissions it is allowed to use, and enables the OAuth token flow required for all SharePoint Embedded API calls. This ensures that only authorized applications can create, manage, and access container types and their content, providing a consistent and governable security model for developers and administrators.
...
4. Add the following Graph API permissions, ensuring they have admin consent.

    - FileStorageContainer.Selected - required for your application to make SharePoint Embedded Graph API Calls
    - FileStorageContainerType.Manage.All - Required to create and register container types in SharePoint Embedded
    - FileStorageContainer.Manage.All - Required to create and managed containers in SharePoint Embedded

    ![alt text](../images/CTCreationGraphPerms.png)
...
1.  Standard Container Type - defines the structured, production‑ready storage blueprint that applications use to create containers with consistent permissions, policies, and lifecycle behavior at scale.

> [!NOTE]
> When executing the Graph commands to configure container types, the [Graph Explorer](https://learn.microsoft.com/en-us/graph/graph-explorer/graph-explorer-overview?view=graph-rest-1.0%2F%3FWT.mc_id%3Dmsgraph_inproduct_graphexhelp) can be used with the permissions granted in the Azure App Registration.


The following Graph command will create a new standard container type.  Replace the owningAppId with the Azure App Registration Application (client) ID, created above.
...
```
The following response will be returned once the Container Type is created:

> [!NOTE]
> The id value is the Container Type ID, record this for future operations.
