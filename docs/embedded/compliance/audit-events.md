...
SharePoint Embedded operations on container types are captured in the Microsoft 365 unified audit log through [Microsoft Purview](/purview/audit-solutions-overview). These events let compliance administrators and developers track changes to container type definitions and their tenant registrations.
...
To search for SharePoint Embedded audit events:

1. Go to the [Microsoft Purview portal](https://purview.microsoft.com) and select **Audit**.
1. On the **Search** page, filter by the relevant activity category:
   - **SharePoint Embedded Container Type activities** for container type events.
   - **SharePoint Embedded Container Type Registration activities** for registration events.
...
- [Consuming Tenant Admin](../administration/consuming-tenant-admin/ctaUX.md)
