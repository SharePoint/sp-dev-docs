---
title: Hands on Lab - Setup & configure the SharePoint repository services environment and web app project
...
---
# Hands on Lab - Setup & configure the SharePoint repository services environment and web app project

In this exercise, you’ll create a project that contains a server-side and client-side component. The server-side component is a REST API that authenticates with Microsoft Entra and will access SharePoint repository services Containers using the Microsoft Graph API. The client-side component is a single-page React app that will call the REST API that exposes the contents of the SharePoint repository services Container. At the end of this exercise, you’ll have a template project that you’ll use to add additional functionality in the following sections.
...
## Create a web app to access SharePoint repository services Containers

With SharePoint repository services setup, the next step is to create the application. This project consists of two projects:
...
- **API_ENTRA_APP_CLIENT_ID**: This is the application (client) ID of the Microsoft Entra application you created previously.
- **API_ENTRA_APP_CLIENT_SECRET**: This is the application (client) secret of the Microsoft Entra application you created previously.
- **API_ENTRA_APP_AUTHORITY**: This is the authority of the Microsoft Entra application. Use `https://login.microsoftonline.com/common/`.
...
- **CLIENT_ENTRA_APP_CLIENT_ID**: This is the application (client) ID of the Microsoft Entra application you created previously.

    > [!NOTE]
    > In this module, this is the same value as the **API_ENTRA_APP_CLIENT_ID** as we’re using the same Microsoft Entra application for configuring the SharePoint repository services app and the web application. In a production scenario, these can be two different Microsoft Entra applications.

- **CLIENT_ENTRA_APP_AUTHORITY**: This is the authority of the Microsoft Entra application. Use `https://login.microsoftonline.com/common/`.
- **CONTAINER_TYPE_ID**: This is the ID of the SharePoint repository services Container Type that you created when setting up your environment previously.
...
// SharePoint repository services scopes
...
In this exercise, you created a project that contains a server-side and client-side component. The server-side component is a REST API that authenticates with Microsoft Entra and will access SharePoint repository services Containers using the Microsoft Graph API. The client-side component is a single-page React app that will call the REST API that exposes the contents of the SharePoint repository services Container.

## Next Steps

Let's get started and continue with the next topic in this tutorial, [Create REST API to handle read and display operations with SharePoint repository services](./m02-04-unit.md), where you'll learn how to access SharePoint repository services Containers through the Microsoft Graph REST APIs.
