...
# Scenarios and use cases for SharePoint repository services 

Use these example scenarios to prompt ideas about how custom applications can use SharePoint repository services. 

> [!NOTE]
> This article is not intended to be an exhaustive list of all SharePoint repository services features and scenarios. The intention is that these scenarios are contextualised examples of how combinations of features can be used.
...
Where your application is enabling a business-critical or time sensitive process, use the dedicated resource allocation of SharePoint repository services to simplify management of throttling. 
...
### Why use SharePoint repository services instead of SharePoint? 
...
Because SharePoint repository services is headless, there is no user interface other than what is provided by your custom application. If you do not supply a method to change content or settings through your application, then it won’t be possible for a user to circumvent this through SharePoint. You have the choice for which collaborative features are available in your application- for example sharing. 
...
Using SharePoint repository services will support this scenario, while not requiring the users of your application to have access or entitlement to your Microsoft 365 tenant. 
