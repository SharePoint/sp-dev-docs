...
This trigger can be used to initiate a Power Automate flow when a supported form experience in SharePoint receives a response. Supported form types include list forms, file upload forms, and document generation forms. This enables customers to automate downstream processes such as routing submissions for approval, sending notifications, or initiating document workflows based on submitted responses.

The trigger outputs submission‑level information such as the form identifier, submission identifier, responder information, submission time, and the responses captured for the form questions. For form experiences that support file uploads or document generation, the trigger also provides metadata about uploaded or generated documents where applicable.
...
Use this action to generate documents by mapping template fields to corresponding content from supported data sources such as SharePoint lists, Excel, or dynamic values provided within a flow. This allows customers to automate document creation scenarios using published document generation forms as part of business processes.

The generated output includes file metadata such as the Name, link, location, and generation status of the created document. This action is supported only for document generation (DocGen) forms.

Document generation forms are part of *AI in SharePoint Public Preview* and are available only to customers enrolled in the preview program.
...
Gets info about the form. Use this action to obtain the metadata and schema definition of a selected form before executing downstream automation logic in a flow. The returned metadata includes information such as the form identifier, title, link, form type, defined output format, and the questions used to collect responses.

This action can be used in automation scenarios where the structure of the form must be evaluated dynamically at runtime before performing additional operations such as validation, transformation, or routing of collected data.
