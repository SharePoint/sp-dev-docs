...
This is the eighth in a series of articles about the basics of developing provider-hosted SharePoint Add-ins. You should first be familiar with [SharePoint Add-ins](sharepoint-add-ins.md) and the previous articles in this series, which you can find at [Get started creating provider-hosted SharePoint Add-ins](get-started-creating-provider-hosted-sharepoint-add-ins.md#next-steps).
[Next steps](get-started-creating-provider-hosted-sharepoint-add-ins.md#next-steps)
...
1. In **File Explorer**, go to the solution folder, the **ChainStoreWeb** folder, and then open the **Utilities** folder.
1. Select SharePointComponentDeployer.cs, and then select **Add**.
...
1. At the top of the `SharePointComponentDeployer` class, add the following two static fields. Both of these are initialized in the **Page_Load** method of the add-in's start page (you add that code in a later step).
...
1. Create the following static property to hold the version of the add-in that is currently recorded in the corporate **Tenants** table. It uses the two methods that were already in the file to get and set this value.
...
1. Now create the following `IsDeployed` property.
...
1. Open the CorporateDataViewer.aspx.cs file and add the following code to the **Page_Load** method, just under the line that initializes the `spContext` object.
...
1. Replace `TODO6` with the following code, which changes the public name of the "Title" field (column) from "Title" to "Name." This is what you did on the **List Settings** page when you created the list manually.
...
1. You also manually created a field named **Added to Corporate DB**. To do that programmatically, add the following code in place of  `TODO7`.
...
1. The entire `CreateLocalEmployeesList` should now look like the following.
...
1. On the **Permissions** tab, leave the **Scope** value at Web, but in the **Permission** field, select **Manage** from the drop-down.
1. Save the file.
...
1. When the add-in's start page opens, select the **Back to Site** link on the chrome control at the top.
1. Go to the **Site Contents** page. The **Local Employees** list is present because your first-run logic added it.
...
1. Open the list and add an item. Note that on the new item form, the **Added to Corporate DB** field is no longer present, so it cannot be manually set. This is true of the edit item form as well.
...
1. Use the browser's back button to go back to the add-in's start page.
1. Select the gear icon on the chrome control at the top, and then select **Account settings**.
1. On the **Account settings** page, select the **Show Add-in Version** button. The version shows as **1.0.0.0** because the first-run logic changed it.
