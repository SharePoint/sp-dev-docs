...
1. Go to `http://<SharePointWebSite>/_layouts/15/AppInv.aspx`, where \<SharePointWebSite\> is the URL of the website where the add-in is installed. Be careful not to add any query parameters on the URL. The form you need only appears on this page if the URL is exactly as shown.
