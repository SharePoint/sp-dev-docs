...
>**Note:** GraphHttpClient is currently in preview and is subject to change. It is not currently supported for use in production environments.
...
This article describes how to call the Microsoft Graph API from a simple SharePoint web part. You will use the **get()** method to make a REST call to the Microsoft Graph to retrieve a list of Office 365 groups. 
...
* Enter **y** when prompted if you want to allow the tenant admin the choice of being able to depoy the solution to all sites immediately without running any feature deployment or adding apps in sites.
...
The Yeoman generator will build the web part. When the scaffolding completes, open your project folder in your code editor. This article uses Visual Studio Code in the steps and screenshots but you can use any editor you prefer.

Run the gulp serve command and confirm it runs in the local workbench correctly
...
First you need to modify the HTML to provide a button to retrieve Office 365 groups. The HTML also needs a placeholder to display the groups.

In your code editor, open the **/src/webparts/helloGraph/HelloGraphWebPart.ts** file.

Modify the **render()** method so that it contains a button and a div where the code will list the Office 365 groups after they are retrieved.
...
Next, you need to define an interface to represent each Office 365 group. Add the following code just above the HelloGraphWebPart class but below the imports.

```typescript
export interface IOffice365Group {
  //The Microsoft Graph has more group properties, but we'll just track the following three
  displayName: string;
  mail: string;
  description: string;
}
...
### Use GraphHttpClient.get method to retrieve Office 365 groups
...
Import the **GraphHttpClient** class and related types by adding the following import statement near the top of the **HelloGraphWebPart.ts** file.

```typescript
import { GraphHttpClient, HttpClientResponse, IGraphHttpClientOptions } from '@microsoft/sp-http';
```

Now create the _readGroups() method by adding the following code to the **HelloGraphWebPart** class.
...
In the previous code, the context property has the GraphHttpClient instance. By calling the **get()** method, a REST call is made to the Microsoft Graph passing the specified URL. In this case the URL is **v1.0/groups?orderby=displayName**. This issues a GET request and the Microsoft Graph will return all Office 365 groups in the tenant in order of display name.

You can issue any GET request using this technique and entering the correct URL values which are documented in the [Microsoft Graph documentation](https://developer.microsoft.com/en-us/graph/docs/concepts/overview). For example, the Groups GET requests are documented in [Get group](https://developer.microsoft.com/en-us/graph/docs/api-reference/v1.0/api/group_get).
...
Now create a **_renderTable()** method to render the returned groups in a table where each row represent a group. Add the following method to the **HelloGraphWebPart** class.
...
### Running the web part to call the Microsoft Graph
...
Use gulp to package your solution.

```
gulp package-solution
```

Deploy the solution to your SharePoint tenant:
...
Use gulp serve to host the web part.

```
gulp serve --nobrowser
```

Add the web part to a web page, or use the SharePoint workbench.
...
Modify the **render()** method so that it contains a button and a div that will indicate wheter the creation was successful.
...
Then add the **_createGroup()** method to call the Microsoft Graph API and create a new group.
...
### Running the web part to create a new group
...
When you choose **Create New Group**, the code will create a new Office 365 group. 

>**Note:** If you try to create the same group again, the code will get an error because it already exists. The error is logged to the console, which you can view in the browser's developer mode.
