...
Now, rename the **EmptyBot.cs** file with name **CollectFeedbackBot.cs**, open it and replace the base class from `ActivityHandler` to `SharePointActivityHandler` and empty the content of the class itself. You also need to import the namespace `Microsoft.Bot.Builder.SharePoint`. Then, add a set of overrides for all the methods with name starting with `OnSharePointTask*`.
...
Now create a new folder with name **TeamsAppManifest** in the root folder of your project and add a couple of image files for the icons of the Microsoft 365 application that you're defining and a **manifest.json** file.
