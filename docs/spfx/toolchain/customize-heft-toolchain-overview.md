---
title: TODO_TODO_TODO_TODO_TODO_TODO_TODO_TODO
description: In this article, you'll learn TODO_TODO_TODO_TODO_TODO_TODO_TODO_TODO
ms.date: 11/18/2025
ms.localizationpriority: high
---
# TODO_TODO_TODO_TODO_TODO_TODO_TODO_TODO

<!--TODO: overview -->

<!--TODO: recap scenarios & things we did in gulp-based toolchain... include links to the gulp-toolchain overview page + links to those pages (custom tasks, webpack plugins, etc.) -->

<!--TODO: how it works -->

<!--TODO: phase, tasks, task config, rig, rig plugins -->

<!--TODO: what can you do with heft -->

<!--TODO: high level list of scenarios with links to other pages in this PR -->

<!--TODO: list all commands (phases) avail -->

```console
heft -h
```

## See also

- [SharePoint Framework Toolchain: Rush Stack, Heft, & Webpack](sharepoint-framework-toolchain-rushstack-heft.md)
- [Migrate from the Gulp Toolchain to Heft Toolchain](migrate-gulptoolchain-hefttoolchain.md)
<!--TODO: add other customization pages -->
