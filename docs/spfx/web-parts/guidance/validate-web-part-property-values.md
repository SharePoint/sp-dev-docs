...
> [!NOTE]
> The Yeoman generator creates environment-related string properties (like `AppLocalEnvironmentSharePoint`, `AppTeamsTabEnvironment`, etc.) that are used to display messages about where the web part is running. These properties are essential for the web part to function correctly. When following this tutorial, ensure all these properties remain in your file and only verify that the `ListNameFieldLabel` property has been added.
...
1. Change the code of the **./src/webparts/listInfo/loc/en-us.js** file to:
    ...
    ```
> [!NOTE]
> The environment-related string values shown above are required by the web part to display contextual messages about the running environment (SharePoint, Teams, Office, Outlook, etc.). Keep all these values in your file as they were generated by the Yeoman scaffolding tool.
...
### Validate the name of the list with the SharePoint REST API
