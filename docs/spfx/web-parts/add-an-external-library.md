...
### Example
Let's include the string validating library [validator](https://www.npmjs.com/package/validator) package into a web part.
...
As we are loading the library from CDN, we do not need to install the npm package. However, we still need to install the typings. 
...
1. Add or create tpyings for this class. In this case, we will create `Contoso.d.ts`, which contains typings for both JavaScript files. 
