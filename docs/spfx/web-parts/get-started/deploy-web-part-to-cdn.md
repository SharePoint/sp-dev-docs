...
1. Create a CDN endpoint as described in [Step 2: Enable CDN for the storage account](/azure/cdn/cdn-create-a-storage-account-with-cdn#step-2-enable-cdn-for-the-storage-account). The CDN endpoint is created with the following URL: **`http://spfxsamples.azureedge.net`**
...
1. Switch to the console and execute the following command:

    ```console
    heft build --production
    ```

    This builds the minified assets required to upload to the CDN provider. The **--production** argument indicates the build tool to build for distribution.

    The minified assets can be found under the **release/assets** directory.

## Deploy assets to Azure Storage

1. Switch to the console of the **azurehosted-webpart** project directory.
1. Enter the following command to deploy the assets to your storage account:

    ```console
    heft deploy-azure-storage --production
    ```
...
Because you changed the web part bundle, you need to redeploy the package to the app catalog. You used **--production** to generate minified assets for distribution.

1. Switch to the console of the **azurehosted-webpart** project directory.
1. Enter the following command to package the client-side solution, this time with the **--production** flag set. This forces the task to pick up the CDN base path configured in the previous step:
...
1. Notice that you aren't running **heft start**, and therefore nothing is served from **localhost**. Content is served from the Azure CDN. You can also double-check this by selecting <kbd>F12</kbd> in your browser and confirming that you can see the Azure CDN as one of the sources for the page assets.

    ![Sources with Azure CDN URL](../../../images/cdn-azure-sources-browser-dev-tools.png)

## Deploy to other CDNs

To deploy the assets to your favorite CDN provider, you can copy the files from **release/assets** folder. To generate assets for distribution, run the following command as we did before with the **--production** parameter:
