...
SharePoint Framework solutions consist only of client-side code. If your solution requires server-side code—for example, to run long-running operations, scheduled processes, or connecting to other systems that don't support the OAuth implicit flow—you need to build this functionality separately and expose it through an API secured with Microsoft Entra ID. Your SharePoint Framework solution would then [securely connect to this API on behalf of the current user](use-aadhttpclient.md).
