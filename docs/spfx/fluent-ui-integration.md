---
title: Using Fluent UI in SharePoint Framework
description: Information about using Fluent UI React and Fabric Core styles in SharePoint Framework, including CSS challenges and migration guidance.
ms.date: 03/23/2026
ms.localizationpriority: high
---

# Using Fluent UI in SharePoint Framework

> [!NOTE]
> **Office UI Fabric has been renamed to Fluent UI.** Starting with SPFx v1.18, new projects use the `@fluentui/react` (v8) package. Older SPFx projects may still reference the legacy `office-ui-fabric-react` package. Both package names are supported. For more information, see [Fluent UI](https://developer.microsoft.com/fluentui).

Fluent UI (formerly Office UI Fabric) is the official front-end framework for building experiences in Microsoft 365 and SharePoint. SharePoint provides seamless integration with Fluent UI that enables Microsoft to deliver a robust and consistent design language across various SharePoint experiences such as modern team sites, modern pages, and modern lists. Additionally, Fluent UI is available for developers in the SharePoint Framework when building custom SharePoint solutions.

Microsoft uses Fluent UI in SharePoint. Microsoft regularly pushes updates to SharePoint Online that could also update the Fluent UI versions as well. These updates could potentially conflict with third-party customer solutions built with previous versions of Fluent UI (or the legacy Office UI Fabric packages), which could cause exceptions in those customizations. The primary reason for these types of breaks is the use of **Global CSS styles** in both Fabric Core and Fluent UI React libraries. Such conflicts need to be avoided at all costs.
...
To achieve reliability, one of the main problems we need to solve is that of **Global CSS styles**. Use CSS Modules (`.module.scss` files, which SPFx scaffolds by default) or Fluent UI styling APIs (`mergeStyles`, `makeStyles`) instead of global Sass mixins. If you're maintaining a legacy project that still relies on Fabric Core Sass declarations, import them through the scoped `@microsoft/sp-office-ui-fabric-core` package rather than referencing global class names directly in HTML markup.
...
- Customers should be able to smoothly and reliably consume Fabric Core and Fluent UI React in their solutions.
- Microsoft will roll out updated experiences that consume updated versions of Fluent UI in SharePoint without conflicting with customer's solutions.
- Customers can customize and override the default styles, designs, and components to tailor the solution's needs.

There are two parts of Fluent UI that are available for use by developers:

- [Fluent UI Core Styles](https://developer.microsoft.com/fluentui). A set of core styles, typography, a responsive grid, animations, icons, and other fundamental building blocks of the overall design language (formerly Office UI Fabric Core).

- [Fluent UI React](https://developer.microsoft.com/fluentui#/controls/web). A set of React components built on top of the Fluent UI design language for use in React-based projects (formerly Office UI Fabric React).

## Fabric Core styles package
...
## Fluent UI React (formerly Office UI Fabric React)

We recommend that you use the version of the Fluent UI React package included in your SharePoint Framework project by the Yeoman generator.

- **SPFx v1.18 and later**: Projects use the `@fluentui/react` (v8) package.
...
For **SPFx v1.18+** projects using `@fluentui/react`:
...
For **legacy** projects using `office-ui-fabric-react`:
...
The Fluent UI React package includes the supported core styles used in the components. We recommend that you import the core styles from the Fluent UI React package instead of from the **\@microsoft/sp-office-ui-fabric-core** package to ensure that the right styles are used in your component.

Because the **\@microsoft/sp-office-ui-fabric-core** package is already installed in your solution by the Yeoman generator, we recommend that you uninstall that package if you decide to use Fluent UI React components to reduce your component bundle size.

```console
npm uninstall @microsoft/sp-office-ui-fabric-core --save
```

You can then import the core styles from the Sass declarations available in the Fluent UI React package.
...
For legacy `office-ui-fabric-react`:
...
The following concepts and references provide insights on the challenge with using Fluent UI (formerly Office UI Fabric) in the context of client-side web parts.
...
There are changes on how to use Fluent UI (formerly Office UI Fabric) icons in the rendering of the SharePoint Framework solutions starting from the SharePoint Framework v1.8.2.
...
Solutions built with *no JavaScript framework* option.

For **SPFx v1.18+** using `@fluentui/react`:
...
For **legacy** projects using `@uifabric/styling`:
...
Solutions built with *React* option or by using *React* in general.
