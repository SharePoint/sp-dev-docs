...
Typically, when building Microsoft Teams tab, you need to [build custom UI to allow users to configure your tab](https://docs.microsoft.com/en-us/microsoftteams/platform/tabs/how-to/create-tab-pages/configuration-page). Additionally, you need to write code to store and load configuration values as selected by the user.
...
Microsoft Teams personal apps don’t offer any infrastructure for implementing configuration. Instead, following the pattern recommended by Microsoft Teams, personal app’s [settings should be exposed in a separate tab](https://docs.microsoft.com/en-us/microsoftteams/platform/concepts/design/personal-apps).

:::image type="content" source="../images/build-for-teams/build-for-teams-personal-app.png" alt-text="Personal app with multiple tabs including settings built using SharePoint Framework":::
...
:::image type="content" source="../images/build-for-teams/build-for-teams-manifest-setttings-webpart.png" alt-text="The supportedHosts property of a SharePoint Framework web part used as a personal app configuration UI":::
...
:::image type="content" source="../images/build-for-teams/build-for-teams-teams-manifest-personalapp-multipletabs.png" alt-text="Microsoft Teams app's manifest defining a personal app with multiple tabs pointing to SharePoint Framework web parts":::
...
One of the less-known options for persisting application- and user-specific configuration is using the [application’s personal folder](https://docs.microsoft.com/en-us/graph/api/drive-get-specialfolder?view=graph-rest-1.0&tabs=http). Application’s specific folder is located in the user’s OneDrive for Business site. Each application gets a designated folder in which it can store any number of files.

:::image type="content" source="../images/build-for-teams/build-for-teams-application-personal-folder.png" alt-text="Application’s personal folder created for SharePoint Framework applications":::
